(function(window) {
  ...
  var baseArgs = [
    ...
    eventInfo = {
      ...
      ClipboardEvent: {
        args: [{ name: "dataType", defaultValue: "" }, { name: "data", defaultValue: "" }]
      },
      CloseEvent: {
        args: [
      ...
      CustomEvent: {
        args: [{ name: "detail", defaultValue: null }]
      },
      DeviceMotionEvent: {
        args: [
      ...
      GamepadEvent: {
        args: [{ name: "gamepad", defaultValue: null }]
      },
      HashChangeEvent: {
        args: [{ name: "oldURL", defaultValue: "" }, { name: "newURL", defaultValue: "" }]
      },
      KeyboardEvent: {
        args: [
          { name: "view", defaultValue: null },
      ...
      MediaStreamEvent: {
        args: [{ name: "stream", defaultValue: null }]
      },
      MessageEvent: {
        args: [
      ...
      PageTransitionEvent: {
        args: [{ name: "persisted", defaultValue: false }]
      },
      PointerEvent: {
        args: [
          { name: "view", defaultValue: null },
      ...
      PopStateEvent: {
        args: [{ name: "state", defaultValue: null }]
      },
      ProgressEvent: {
        args: [
      ...
      UIEvent: {
        args: [{ name: "view", defaultValue: null }, { name: "detail", defaultValue: 0 }]
      },
      UserProximityEvent: {
        args: [{ name: "near", defaultValue: false }]
      },
      WebGLContextEvent: {
        args: [{ name: "statusMessage", defaultValue: "" }]
      },
      WheelEvent: {
        args: [
          { name: "view", defaultValue: null },
  ...
  function createEvent(info, thisArg, args) {
    ...
    if (thisArg === info.proto || !(thisArg instanceof info.defaultConstructor)) {
      throw new TypeError(
        "The '" + info.name + "' constructor requires use of the 'new' operator."
      );
    }
    ...
    if (args.length === 0) {
      throw new TypeError(
        "Failed to construct '" + info.name + "': parameter 1 ('type') is required."
      );
    }
    if (args.length > 1 && args[1] != null && !isObject(args[1])) {
      throw new TypeError(
        "Failed to construct '" + info.name + "': parameter 2 ('eventInitDict') is not an object."
      );
    }
    type = "" + args[0];
  ...
  function polyfillEvent(info, interfaceName) {
    ...
    if (
      !eventInterface ||
      (eventInterface.prototype !== Event.prototype &&
        !(eventInterface.prototype instanceof Event)) ||
      canConstructEvent(eventInterface)
    ) {
      return false;
    }
    ...
    while (!initializer && i < l) {
      ...
      initializer = isObject(eventInterface.prototype)
        ? eventInterface.prototype[initializerName]
        : null;
    }
  ...
  for (var interfaceName in eventInfo) {
    if (
      eventInfo.hasOwnProperty(interfaceName) &&
      !polyfillEvent(eventInfo[interfaceName], interfaceName)
    ) {
